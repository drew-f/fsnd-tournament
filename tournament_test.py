...
def runTournament():
    """
    Run a sample tournament with 16 players, with the winner of each match
    determined randomly.
    """
    deleteMatches()
    deletePlayers()
    print "11. Running test tournament with 16 players."
    new_players = ["Amy Anderson", "Billy Bennett", "Celia Cameron",
                   "Daniel Davis", "Emily Ellis", "Franklin Fischer",
                   "George Gale", "Henry Harris", "Isaac Irwin",
                   "Jennifer Jones", "Kate Kelly", "Linda Larsen",
                   "Megan Masters", "Nathan Nelson", "Olivia Olson",
                   "Peter Page"]
    for player in new_players:
        registerPlayer(player)
    total_players = countPlayers()
    print "Registered Players: %i" % (total_players, )
    rounds_req = roundsRequired()
    print "Rounds Required %i" % (rounds_req, )
    rounds_completed = 0
    while (rounds_completed < rounds_req):
        pairings = swissPairings()
        ...
        rounds_completed += 1
    print "Tournament completed in: %i rounds" % (rounds_completed)


def printStandings():
    """
    Print the standings of all registered players.
    """
    standings = playerStandings()
    for player in standings:
        print "%s: %i Wins from %i Matches" % (player[1], player[2], player[3])
